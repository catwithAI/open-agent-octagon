...
class OctagonEvalsClient:
    ...
    def score(
        ...
        *,
        ...
        evidence = {
            "task": task,
            "final_state": _read_json(attempt_root / "final_state.json"),
            "trace": _read_jsonl(
                attempt_root / "trace.jsonl",
                max_bytes=max(4_096, self.config.max_evidence_bytes // 4),
            ),
            "events": _read_jsonl(
...
def _read_jsonl(path: Path, *, max_bytes: int | None = None) -> list[dict[str, Any]]:
    ...
    for line in lines:
        ...
        if max_bytes is not None and result and used + encoded_size > max_bytes:
            break
        result.append(value)
        used += encoded_size
    return result
